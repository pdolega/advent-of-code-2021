...
class PilotSubmarineTest extends AnyFlatSpec {
  "Pilot submarine" should "work correctly with 0 commands" in {
    assert(new PilotSubmarine().calculateMultlication(LazyList()) == 0)
  }

  "Pilot submarine" should "work correctly with 1 commands" in {
    assert(new PilotSubmarine().calculateMultlication(LazyList("forward 5")) == 0)
  }

  "Pilot submarine" should "work correctly with horizontal and depth command" in {
    assert(new PilotSubmarine().calculateMultlication(LazyList("forward 5", "up 3", "down 5")) == 0)
  }

  "Pilot submarine" should "work correctly with aim changed initially" in {
    assert(new PilotSubmarine().calculateMultlication(LazyList("up 3", "forward 5", "down 5", "forward 5")) == -50)
  }

  "Pilot submarine" should "fail when unknown command provided" in {
    assertThrows[Error](new PilotSubmarine().calculateMultlication(LazyList("xxx 5")))
  }

  // actual exercise
  "Pilot submarine" should "work for test input" in {
    val result = new PilotSubmarine().calculateMultlication(DataUtils.getDataLines(getClass))
